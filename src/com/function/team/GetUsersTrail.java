...
public class GetUsersTrail {
	
	
	public String[][] doInBackground(String tid,String uid)
	{
		String json="";
		CharSequence teamid="";
		CharSequence usersid="";
		String[][] rest=null;
		teamid=tid.toString();
		usersid=uid.toString();
		try{
			//Toast.makeText(a, "3", Toast.LENGTH_SHORT).show();
			HttpPost httpRequest =new HttpPost(HttpUtil.GETUSERSTRAIL_URL);
			//请求数据
			...
			params.add(new BasicNameValuePair("uid",usersid.toString()));
			...
			if(httpResponse.getStatusLine().getStatusCode()==200){
				...
				try {
					...
					for (int i = 0; i < jsonArray.length(); i++) {  						
						...
						// 获取对象对应的值      
						rest[i][0]= item.getString("userslat");   
						rest[i][1]= item.getString("userslon");
						rest[i][2]= item.getString("usersname");
		            }   
