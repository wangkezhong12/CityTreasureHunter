...
public class TimeEnd extends AsyncTask{
	...
	protected Object doInBackground(Object... params_obj)
	{
		...
		CharSequence cnum="";
		CharSequence sign="";
		tuid=params_obj[0].toString();
		sid=params_obj[1].toString();
		cnum=params_obj[2].toString();
		sign=params_obj[3].toString();;
		
		
		try{
			HttpPost httpRequest =new HttpPost(HttpUtil.STARTORENDGAME_URL);
			if(!tuid.equals("")){
				...
				params.add(new BasicNameValuePair("tuid",tuid.toString())); 
				params.add(new BasicNameValuePair("sid",sid.toString())); 
				params.add(new BasicNameValuePair("cnum",sid.toString())); 
